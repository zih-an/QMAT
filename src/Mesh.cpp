...
void MPMesh::computebb()
{
    ...
    if (isCoverageAxis)
        bb_diagonal_length = 1; //coverageAxis - DON'T normalize the input geometry. this is modified by Zhiyang. for error checking...
    else
        bb_diagonal_length = sqrt((m_max[0] - m_min[0]) * (m_max[0] - m_min[0]) + (m_max[1] - m_min[1]) * (m_max[1] - m_min[1])
                              +(m_max[2] - m_min[2]) * (m_max[2] - m_min[2]));  //original qmat with normalization.
}
...
void MPMesh::computedt()
{
    ...
    Vertex_iterator pVertex;
    int idx = 0;
    pVertex = vertices_begin();  //zihan note: inherit the Polyhedron_3. use all mesh vertices to compute dt.
    for(; pVertex != vertices_end(); pVertex ++, idx ++)
    {
