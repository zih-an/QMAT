...
void SlabMesh::EvaluateEdgeCollapseCost(unsigned eid){
    //coverageaxis
    if (isCoverageAxis) {
        EvaluateEdgeCollapseCost_coverageaxis(eid);
        return;
    }
    ...
    // 当发生翻转时，选取没有引起翻转的方式进行合并
    Wm4::Vector3d tempVector(lamdar.X(), lamdar.Y(), lamdar.Z());
    if (!Contractible(v1, v2, tempVector))
    {
        int count = 0;
        double *collapse_costs = new double[3];
        Sphere *min_sphere = new Sphere[3];
...
void SlabMesh::EvaluateEdgeCollapseCost_coverageaxis(unsigned eid){
	//Modified by Zhiyang ...
// this is key part of cost assigning...
	...
	//double w1 = 1e-5, w2 = 1e-5;
	double w1 = 1.0, w2 = 1.0;
	//// 锟皆诧拷同ratio锟侥边斤拷锟斤拷映锟戒处锟斤拷小锟斤拷0.2锟侥诧拷锟斤拷锟斤拷锟斤拷(0.2,1)映锟戒到(2, 10)
	//w1 = edges[eid].second->hyperbolic_weight * edges[eid].second->hyperbolic_weight * edges[eid].second->hyperbolic_weight;
	//w2 = w1;
	//if (edges[eid].second->hyperbolic_weight >= 0.2)
	//{
	...
	// 锟斤拷锟斤拷锟斤拷锟斤拷转时锟斤拷选取没锟斤拷锟斤拷锟斤拷转锟侥凤拷式锟斤拷锟叫合诧拷
...
void SlabMesh::readMA_ball_diff_radius(
    string objpath,
...
void SlabMesh::Simplify_with_Selected_Pole(
    int threshold,
    vector<vector<double> >& selected_pole,
    std::string selected_inner_points_file,
    double bb_diagonal_length
){
	...
	*/
	...
	int preserve_num = 30;
    readMA_ball_diff_radius(selected_inner_points_file, vset, eset, fset, bb_diagonal_length); // 49

	// std::ofstream fsout("./all_poles.obj");
